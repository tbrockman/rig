...
**A USB controller for a VM someone sits at.** The same rule applies to any
other device a manifest passes through, and a USB controller is the usual
second one: the monitor's hub plugs into it, and the keyboard and mouse follow
the VM. List the controllers and their groups, then find which one a port
belongs to by plugging something in:

```bash
for d in /sys/bus/pci/devices/*; do
  case "$(cat $d/class)" in 0x0c03*) echo "$(basename $d) group $(basename $(readlink $d/iommu_group))";; esac
done
lsusb -t                                      # bus N -> /sys/bus/usb/devices/usbN -> its controller
readlink -f /sys/bus/usb/devices/usb3 | grep -o '0000:[0-9a-f:.]*' | tail -1
```

A controller alone in its group, with nothing the host needs on it, goes into
`rig.yaml` as `kind: pci`. Prefer one that shares nothing with the host: on
the reference host the CPU-attached controller is a function of the same PCI
device as the iGPU driving the console, and passing it through hung the
machine hard (docs/DESIGN.md, 2026-09-20). A chipset controller often
carries the board's Bluetooth and hubs and cannot go either. When no
controller qualifies, pass the keyboard and mouse by identity (`kind: usb`)
and leave every controller with the host. Which rear port belongs to which
controller is not written anywhere in sysfs.
